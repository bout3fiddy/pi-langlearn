  ...
  async grade(question: Question, userAnswer: string): Promise<GradeResult | null> {
    ...
    try {
      ...
      return {
        ...
        expectedAnswer:
          typeof parsed.expectedAnswer === "string" ? parsed.expectedAnswer : undefined,
        normalizedUserAnswer:
          typeof parsed.normalizedUserAnswer === "string" ? parsed.normalizedUserAnswer : undefined,
      };
    } catch {
      return null;
    }
  ...
  private async createSession(): Promise<any | null> {
    try {
      ...
      const resourceLoader = new DefaultResourceLoader({
        systemPromptOverride: (defaultPrompt) => `${defaultPrompt}${SYSTEM_PROMPT_SUFFIX}`,
      });
