...
export class LearningEngine {
  ...
  getStatus(): EngineStatus {
    ...
    const dueCount = Object.entries(profile.deck.srs).filter(
      ([id, state]) => !profile.deck.suspendedCardIds?.includes(id) && isDue(state, now),
    ).length;
    const newCount = this.deck.filter(
      (card) => !profile.deck.knownCardIds.includes(card.id),
    ).length;
    return {
      lang: profile.lang,
  ...
  private trackRecent(cardId: string): void {
    this.recentCardIds = [cardId, ...this.recentCardIds.filter((id) => id !== cardId)].slice(
      0,
      RECENT_LIMIT,
    );
  }
...
function getQuestionPrompt(question: Question): string {
  switch (question.type) {
    case "de_het":
      return `Article: __ ${question.noun}`;
    case "reorder":
      return `Reorder: ${question.correctSentence}`;
    case "multiple_choice":
    case "type_answer":
    case "cloze":
...
function shouldIncludeTranslationHint(question: Question): string | null {
  ...
  if (question.type === "multiple_choice" || question.type === "cloze") return null;
  if (
    "prompt" in question &&
    typeof question.prompt === "string" &&
    question.prompt.includes(translation)
  )
    return null;
