...
export function loadConfig(): LangLearnConfig {
  ...
  return {
    overlayWidthPercent: clampInt(
      merged.overlayWidthPercent,
      DEFAULT_CONFIG.overlayWidthPercent,
      20,
      90,
    ),
    overlayHeightPercent: clampInt(
      merged.overlayHeightPercent,
      DEFAULT_CONFIG.overlayHeightPercent,
      30,
      80,
    ),
    overlayMargin: clampInt(merged.overlayMargin, DEFAULT_CONFIG.overlayMargin, 0, 6),
    overlayAnchor:
      merged.overlayAnchor === "top-left" || merged.overlayAnchor === "center"
        ? merged.overlayAnchor
        : "top-right",
