...
export default function (pi: ExtensionAPI): void {
	...
	pi.registerCommand("langlearn", {
		description: "Enable language learning or turn it off. Usage: /langlearn <language-name> | /langlearn off",
		handler: async (args, ctx) => {
			...
			if (langArg === "off") {
				...
				if (ctx.hasUI) {
					ctx.ui.notify(enabled ? "Language learning enabled" : "Language learning disabled", "info");
				}
				return;
			}
			...
			if (ctx.hasUI) {
				...
				const message = result.switched ? `Language learning enabled for ${name}` : `${name} learning enabled`;
