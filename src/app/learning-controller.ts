...
export class LearningController {
	...
	enableLanguage(language: LanguageDefinition): { enabled: boolean; switched: boolean; language: LanguageDefinition } {
	...
	ensureOverlayOpen(ctx: any): void {
		...
		const overlayPromise = ctx.ui.custom<OverlayResult>(
			(tui, theme, _kb, done) => {
	...
	renderWidget(width: number, theme: Theme): string[] {
		...
		const label = status.enabled ? "LangLearn" : "LangLearn (off)";
	...
	private setLanguage(language: LanguageDefinition): void {
		...
		this.engine = new LearningEngine(this.store, this.contentStore.getDeck(), () => this.requestWidgetRender());
	}
